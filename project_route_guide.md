...
The API routes in Next.js act as a middleware controller:
1. **GET Requests (`/api/events`, `/api/categories`):**
   - Query the **Strapi CMS** backend.
   - If Strapi is offline/errors, they automatically query the local **PostgreSQL database using Prisma**.
2. **POST/PUT/DELETE Requests (`/api/events`, `/api/events/[slug]`):**
   - Write/update the event in **Strapi CMS** (if running).
   - Fallback or sync updates locally in the PostgreSQL database using Prisma ORM.

## Interaction summary

1. Visitors browse the home and events pages (navbar navigation only points to active public routes).
2. Users register or sign in to access dashboard features.
3. Authenticated users can RSVP, save favorites, and submit events (written locally to PostgreSQL via Prisma).
