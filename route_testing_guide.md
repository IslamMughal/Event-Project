...
## 8. Strapi Integration Testing

### Fallback Mode (Strapi Offline)
1. Stop the Strapi backend if it is running (ensure no process is listening on `http://localhost:1337`).
2. Refresh the Events page (`/events`) or make a GET request to `/api/events`.
3. Check the server console log. It should show: `Strapi fetch failed, using Prisma fallback: ...`.
4. Verify that event and category lists still display correctly using local data fetched via Prisma.

### Connected Mode (Strapi Online)
1. Navigate to the `strapi-backend/` folder and run `npm run develop` to start the Strapi server.
2. In another terminal, restart the Next.js dev server.
3. Query `/api/events`. It should successfully retrieve events from Strapi (with a successful connection and no fallback warning in the server terminal logs).
